...
{
    public class GetCrawlJobDataBehaviour : NavisionProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(NavisionConstants.KeyName.ApiKey, nameof(NavisionCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task NavisionCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Navision.Core.NavisionCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
